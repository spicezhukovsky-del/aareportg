...
def currunt_month(data_dict, data_start, data_end):
    ...
    currunt_month = datatime_filter(filterest_sh, data_start, data_end)
...
def past_month(data_dict, data_start, data_end):
    ...
    past_month = datatime_filter(filterest_sh, data_start, data_end)
...
def create_one_sh_raport(name_doc, name_dict: dict, name_sh, data_dict: dict, data_start: str, data_end_current_month: str, data_end_past_month: str):
    ...
    for id_s, name in name_dict.items():
        if name == name_sh:
            id_sh = id_s
            break
    ...
    try:
        dict_one_sh = rank_all_sh_sort[id_sh]
        coach_one_sh = coach_data_currunt_month[id_sh]
    except KeyError:
        return f"Мониторинг УТЗ в Школе {name_sh} в период с {data_start} по {data_end_current_month} не осуществлялся."
    ...
    try:
        sh_block(name_doc, dict_one_sh, rank_all_sh_sort, name_sh, id_sh, name_dict, rank_all_sh_sort_past)
    except KeyError:
        sh_block(name_doc, dict_one_sh, rank_all_sh_sort, name_sh, id_sh, name_dict)
